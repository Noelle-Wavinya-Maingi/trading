...
class AccountMoveTradePnl(models.Model):
    ...
    def _update_trade_additional_costs(self):
        ...
        if not self.ele_trade_id:
            _logger.warning(f"No trade on bill {self.name}, skipping")
            return
        if self.state != 'posted':
            _logger.info(f"⏭ Bill {self.name} not posted yet, skipping")
            return
        if self.ele_trade_pnl_processed:
            _logger.info(f"⏭ Bill {self.name} already processed, skipping")
            return
        ...
        for line in self.invoice_line_ids:
            ...
            if line.product_id == trade.product_id:
                _logger.info(
                    f"Skipping trade product line: {line.product_id.name} "
    ...
    def _update_trade_pnl_from_invoice(self):
        ...
        if not self.ele_trade_id:
            _logger.warning(f"No trade found on invoice {self.name}")
            return
        if self.ele_trade_pnl_processed:
            _logger.info(f"⏭ Invoice {self.name} already processed, skipping")
            return
    ...
    def _update_trade_pnl_from_sale_order(self):
        ...
        if not self.ele_trade_id or not self.ele_is_from_sale_order:
            _logger.warning(f"Cannot update from sale order: No trade or not from SO")
            return
        if self.ele_trade_pnl_processed:
            _logger.info(f"⏭ Invoice {self.name} already processed, skipping")
            return
        ...
        if self.state == 'posted':
            ...
            if not sale_order:
                _logger.warning(f"Sale order {self.invoice_origin} not found")
                return
    ...
    def _process_line_level_trades(self):
        ...
        if self.state != 'posted':
            _logger.info(f"⏭ Skipping line-level trades for invoice {self.name} (state={self.state}, not posted)")
            return
        if self.ele_trade_pnl_processed:
            _logger.info(f"⏭ Invoice {self.name} already processed, skipping")
            return
