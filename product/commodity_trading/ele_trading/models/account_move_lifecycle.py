...
class AccountMoveLifecycle(models.Model):
    """create/write/action_post/button_draft overrides that wire invoices and bills into their related trade (ele_trade_id propagation, triggering P&L updates at the right lifecycle moments)."""
    _inherit = 'account.move'
    
    def _reverse_trade_pnl_contribution(self, trade):
        """Reverse this move's already-applied contribution to the trade's additional costs or revenue, and clear the ele_trade_pnl_processed so it can be reprocessed later"""
        self.ensure_one()

        if not self.ele_trade_pnl_processed or not trade:
            return
        ...
        self.ele_trade_pnl_processed = False
    ...
    def write(self, vals):
        ...
        if 'ele_trade_id' in vals:
            for move in self:
                old_trade_ids[move.id] = move.ele_trade_id.id
        ...
        if is_moving_to_posted:
            for record in self:
                ...
                if not record.ele_trade_pnl_processed:
                    has_trade = record.ele_trade_id or any(line.ele_trade_id for line in record.invoice_line_ids)
                    ...
                    if has_trade:
                        if record.ele_is_from_sale_order:
                            ...
                            if len(line_trades) >= 1:
                                record.ele_trade_id = line_trades[0].id
                                record.ele_trade_id._compute_invoice_count()
            ...
            if move.ele_is_from_sale_order and move.invoice_origin:
                ...
                if sale_order and sale_order.ele_trade_id:
                    ...
                    for invoice_line in move.invoice_line_ids:
                        if invoice_line.product_id and not invoice_line.ele_trade_id:
    ...
    def button_draft(self):
        ...
        for move in self:
            move._reverse_trade_pnl_contribution(move.ele_trade_id)
