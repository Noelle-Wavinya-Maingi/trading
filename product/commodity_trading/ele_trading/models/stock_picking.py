...
class StockPicking(models.Model):
    ...
    def _process_incoming_picking(self, picking):
        ...
        try:
            ...
            if not trade:
                _logger.warning(f"No trade found for PO {purchase.name}, skipping lot linking.")
                return
            ...
            if not lot_quantities:
                _logger.warning(f"No lots found in picking {picking.name}")
                return
    ...
    def _process_outgoing_picking(self, picking):
        ...
        try:
            ...
            if not trade:
                _logger.info(f"No trade linked to picking {picking.name}")
                return
            
            if trade.ele_status != 'confirmed' and trade.ele_status != 'open':
                _logger.info(f"Trade {trade.name} is {trade.ele_status}, skipping.")
                return
