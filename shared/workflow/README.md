...
- **`workflow.mixin`** — include on an anchor model (`trading.trade`,
...
- **`workflow.step.mixin`** — include on a vertical's own concrete step
...
- **`workflow.template.mixin`** / **`workflow.template.step.mixin`** — a
  reusable blueprint (header + lines) that generates `workflow.step.mixin`
